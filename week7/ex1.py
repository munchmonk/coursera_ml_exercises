...
class Exercise:
	...
	def emailFeatures(self, word_indices):
		n = 1899	# number of words in vocab.txt
		x = np.zeros(n)
	...
	def run(self):
		...
		with open('Data/emailSample1.txt') as fid:
			...
			word_indices  = self.processEmail(file_contents)
		...
		word_indices  = self.processEmail(file_contents)
		features      = self.emailFeatures(word_indices)
		...
		C = 0.1
		model = self.svmTrain(X, y, C, self.linearKernel)

		# --------------------------------------------------------------------------------------------------------------------------------------------
		p = self.svmPredict(model, X)
	...
	def getVocabList(self):
	    ...
	    :return:
	    """
	    vocabList = np.genfromtxt(('Data/vocab.txt'), dtype=object)
